...
class SelfConsistentMF:
    ...
    def run_fragments(self, mf, nimp, *args, **kwargs):
        ...
        if (not self.ab_initio):
            ...
            fci = vayesta.ewf.EWF(mf, fragment_type='Lowdin-AO', bath_type=None, solver='FCI',
                    make_rdm1=True, make_rdm2=True, bno_threshold=np.inf)
    ...
    def get_energy(self, frags):
        ...
        # For ab initio systems, add nuclear/0 electron energy
        if (self.ab_initio):
            e += self.mf.mol.energy_nuc()
    ...
    def get_e_dmet(self, f):
        ...
        eri = None
        # Reshape ab initio repulsion tensor into correct dimensions
        
        
        
        if self.ab_initio:
            n = (self.mf.get_hcore()[0].shape[0])
            #print(n)
            eri = ao2mo.addons.restore('s1', self.mf._eri, n).reshape(n, n, n, n)
        else:
            eri = self.mf.mol.get_eri()
        #print(self.ab_initio)
        #print(self.mf.get_hcore().shape)
        #print(eri.shape)
        ...
        e2 = np.einsum('ijkl,ijkl->', eri, pdm2)/2
        return e1 + e2
