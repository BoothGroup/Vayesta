...
for hubbard_u in hubbard_u_range:
    ...
    # Run brueck calculation
    
    brueck = SelfConsistentMF(mf, sc_type='brueckner', nelectron_target=ne_target, tvecs=None)
    brueck.kernel(nimp=nimp, mo_coeff=mo_brueck)
    mo_brueck = brueck.mf.mo_coeff
    
    with open('2D-energies-dm-brueck.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "brueck"))
        f.write('%12.1f  %16.10f  \n' % (hubbard_u/hubbard_t, brueck.e_dmet))

    s = nimp // 2
    with open('2D-docc-brueck.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "brueck"))
        f.write('%12.1f  %16.10f \n' % (hubbard_u/hubbard_t, brueck.docc[s]))
        
    with open('2D-convergence-brueck.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "brueck"))
        f.write('%12.1f %16.10s %d \n' % (hubbard_u/hubbard_t,  str(brueck.converged), brueck.iteration))
