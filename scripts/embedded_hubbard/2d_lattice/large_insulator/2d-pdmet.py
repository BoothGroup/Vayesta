...
for u in hubbard_u_range:
    ...
    # Run pdmet calculation
    
    pdmet = SelfConsistentMF(mf, sc_type='pdmet', nelectron_target=ne_target, tvecs=None)
    pdmet.kernel(nimp=nimp, mo_coeff=mo_pdmet)
    mo_pdmet = pdmet.mf.mo_coeff
    
    with open('2D-energies-dm-pdmet.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "pdmet"))
        f.write('%12.1f  %16.10f  \n' % (hubbard_u/hubbard_t, pdmet.e_dmet))

    s = nimp // 2
    with open('2D-docc-pdmet.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "pdmet"))
        f.write('%12.1f  %16.10f \n' % (hubbard_u/hubbard_t, pdmet.docc[s]))
        
    with open('2D-convergence-pdmet.txt', 'a') as f:
        if uidx == 0:
            f.write("%12s %16s %16s %16s %16s \n" % ("#", "nsites", "nimps", "doping", "boundary"))
            f.write("%12s  %16d  %16d %16d %16s \n" % ("# ", (nsite), (nimp), (doping), str(boundary_cond)))
            f.write("%12s  %16s \n" % ("# Hubbard-U", "pdmet"))
        f.write('%12.1f %16.10s %d \n' % (hubbard_u/hubbard_t,  str(pdmet.converged), pdmet.iteration))
