...
for uidx, hubbard_u in enumerate(range(u_min, u_max+1, u_step)):
    ...
    print("===============")
    mol = vayesta.lattmod.Hubbard1D(nsite, nelectron=nsite+doping, hubbard_u=hubbard_u, boundary='PBC')
    ...
    sc_pdmet = SelfConsistentMF(mf, sc_type='pdmet', tvecs=tvecs, nelectron_target=ne_target, diis=False)
    sc_pdmet.kernel(nimp=nimp)#, mo_coeff=mo_pdmet)
    mo_pdmet = sc_pdmet.mf.mo_coeff

    sc_brueck = SelfConsistentMF(mf, sc_type='brueckner', tvecs=tvecs, nelectron_target=ne_target, diis=False)
