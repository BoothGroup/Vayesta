    ...
    def emb(cls, bno_threshold):
        ...
        with emb.sao_fragmentation() as f:
            f.add_atomic_fragment(
                0, nelectron_target=cls.targets[0], nelectron_target_atol=1e-7, nelectron_target_rtol=0
            )
            f.add_atomic_fragment(
                1, nelectron_target=cls.targets[1], nelectron_target_atol=1e-7, nelectron_target_rtol=0
            )
            f.add_atomic_fragment(
                2, nelectron_target=cls.targets[2], nelectron_target_atol=1e-7, nelectron_target_rtol=0
            )
        emb.kernel()
        return emb
