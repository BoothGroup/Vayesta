...
def gen_comparison(hubbard_u, nimp=2):
    ...
    for nsite in sites_to_try:
        ...
        dmet = vayesta.dmet.DMET(
            mf, solver="FCI", charge_consistent=True, maxiter=1, bath_options=dict(bathtype="dmet")
        )
    ...
    ax = plt.gca()
    ax.hlines(
        vayesta.lattmod.bethe.hubbard1d_bethe_energy(1.0, hubbard_u),
        sites_to_try[0],
        sites_to_try[-1],
        label="Bethe Ansatz",
    )
