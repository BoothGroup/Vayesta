...
for d in np.arange(0.5, 3.0001, 0.25):
    ...
    dmet_diis.kernel()
    # Single-shot EDMET
    edmet_oneshot = vayesta.edmet.EDMET(dfmf, solver='FCI', max_elec_err=1e-4, maxiter=1, solver_options=dict(max_boson_occ=2))
    with edmet_oneshot.iao_fragmentation() as f:
        for i in range(0, natom, 2):
            f.add_atomic_fragment([i, i + 1])
    edmet_oneshot.kernel()
    # Full DMET
    edmet_orig = vayesta.edmet.EDMET(dfmf, solver='FCI', charge_consistent=True, max_elec_err=1e-4, maxiter=40,
    ...
    edmet_new = vayesta.edmet.EDMET(dfmf, solver='FCI', charge_consistent=True, max_elec_err=1e-4, maxiter=40,
