...
print("Embedded CCSD with full bath:                     mean cluster size= %.3f  E(tot)= %+16.8f Ha" % (
    emb.get_mean_cluster_size(), emb.e_tot))
print("Embedded CCSD with MP2 bath:                      mean cluster size= %.3f  E(tot)= %+16.8f Ha" % (
    emb_mp2.get_mean_cluster_size(), emb_mp2.e_tot))
print("Embedded CCSD with MP2 bath (no DMET projector):  mean cluster size= %.3f  E(tot)= %+16.8f Ha" % (
    emb_noproj.get_mean_cluster_size(), emb_noproj.e_tot))
print("Embedded CCSD with R2 bath:                       mean cluster size= %.3f  E(tot)= %+16.8f Ha" % (
    emb_r2.get_mean_cluster_size(), emb_r2.e_tot))
print("Embedded CCSD with mixed R2/MP2 bath:             mean cluster size= %.3f  E(tot)= %+16.8f Ha" % (
