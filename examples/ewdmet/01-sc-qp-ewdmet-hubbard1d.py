...
for u in [6]:
    ...
    emb = vayesta.ewf.EWF(mf, solver='FCI', bath_options=dict(bathtype='ewdmet', max_order=1, order=1, dmet_threshold=1e-10), solver_options=dict(conv_tol=1e-15, init_guess='mf', n_moments=nmom_max_fci))
    ...
    ax[0].plot(grid, sf_dynamic, "b-", label="QP-EwDMET (dynamic)")
    ax[1].plot(grid, sf_static, "g-", label="QP-EwDMET (static)")
