    ...
    def _test(cls, key, proj=0, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True):
        ...
        if fcifragtype == 'fullsystem':
            ...
            with emb.iao_fragmentation() as f:
                if store_wf_ccsdtq:
                    fci_frags.append(f.add_full_system(solver='FCI', bath_options=dict(bathtype=bathtype), store_wf_type='CCSDTQ', auxiliary=True))
                else:
                    fci_frags.append(f.add_full_system(solver='FCI', bath_options=dict(bathtype=bathtype), auxiliary=True))
        ...
        if fcifragtype == 'atomic':
            ...
            with emb.iao_fragmentation() as f:
                if store_wf_ccsdtq:
                    fci_frag = f.add_all_atomic_fragments(solver='FCI', bath_options=dict(bathtype=bathtype), store_wf_type='CCSDTQ', auxiliary=True)
                else:
                    fci_frag = f.add_all_atomic_fragments(solver='FCI', bath_options=dict(bathtype=bathtype), auxiliary=True)
                ccsd_frag = f.add_full_system(solver='CCSD', bath_options=dict(bathtype='full'))
...
# FCI fragment bath type=full
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_proj0_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='full', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, -7.988952842115175)

    @pytest.mark.fast
    def test_r_exact_ec_lih_631g_atomicfrags_proj1_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='full', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.fast
    def test_r_regression_ec_lih_631g_atomicfrags_proj2_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='full', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot,  -7.98893732925952)

    @pytest.mark.slow
    def test_r_regression_ec_lih_631g_atomicfrags_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, -7.988952842133734)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_atomicfrags_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_regression_ec_lih_631g_atomicfrags_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot,  -7.98893732924287)


    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj0_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='full', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj1_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='full', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj2_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='full', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_r_exact_ec_lih_631g_fullsystem_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)
...
# uhf tests
    ...
    def test_u_regression_ec_lih_631g_atomicfrags_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, -7.988952849629403)

# A test with nostore
    @pytest.mark.slow
    def test_u_regression_ec_lih_631g_atomicfrags_proj0_fciv_nostore(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=False)
        self.assertAlmostEqual(e_tot, -7.988952849629403)

    @pytest.mark.slow
    def test_u_exact_ec_lih_631g_atomicfrags_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=1, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_u_regression_ec_lih_631g_atomicfrags_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=2, bathtype='full', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_exact_ec_lih_631g_fullsystem_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

# test tailoring
    @pytest.mark.fast
    def test_u_exact_tailor_lih_631g_fullsystem_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='full', fcifragtype = 'fullsystem', mode='tailor', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.fast
    def test_u_exact_tailor_lih_631g_fullsystem_proj0_fciv_nostore(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='full', fcifragtype = 'fullsystem', mode='tailor', store_wf_ccsdtq=False)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.fast
    def test_u_exact_ec_lih_631g_fullsystem_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=1, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)

    @pytest.mark.slow
    def test_u_exact_ec_lih_631g_fullsystem_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=2, bathtype='full', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, fci_e_tot)
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj0_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='dmet', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj1_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='dmet', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj2_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='dmet', fcifragtype = 'atomic', mode='external-ccsdv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot,  -7.988931393747468)

    @pytest.mark.slow
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj0_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj1_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj2_ccsdv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-ccsdv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=0, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=1, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_r_regression_ec_lih_631g_fullsystem_bathype_dmet_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'rhf'), proj=2, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj0_fciv_nostore(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=False)
    ...
    def test_u_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=1, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_regression_ec_lih_631g_atomicfrags_bathype_dmet_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=2, bathtype='dmet', fcifragtype = 'atomic', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_regression_ec_lih_631g_fullsystem_bathype_dmet_proj0_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=0, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
    ...
    def test_u_regression_ec_lih_631g_fullsystem_bathype_dmet_proj1_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=1, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
        self.assertAlmostEqual(e_tot, -7.9889313937395015 )

    @pytest.mark.slow
    def test_u_regression_ec_lih_631g_fullsystem_bathype_dmet_proj2_fciv_store(self):
        e_tot, fci_e_tot = self._test(('lih_631g', 'uhf'), proj=2, bathtype='dmet', fcifragtype = 'fullsystem', mode='external-fciv', store_wf_ccsdtq=True)
