...
class UFCI_WaveFunction(RFCI_WaveFunction):
    ...
    def make_rdm2(self, ao_basis=False, with_dm1=True, approx_cumulant=True):
        ...
        return (
            einsum("ijkl,ai,bj,ck,dl->abcd", dm2[0], *(4 * [moa])),
    ...
    def project_occ(self, projector, inplace=False):
        ...
        #           Rx = a.
        # In practice, it is more stable to just compute the excitation level of each state and divide by it.
        mf_vdensity_op = tuple(
            [np.eye(self.norb[i]) - np.pad(np.eye(self.nocc[i]), ((0, self.nvir[i]),)) for i in [0, 1]]
        )
...
class UFCI_WaveFunction_w_dummy(UFCI_WaveFunction):
    ...
    def __init__(self, mo, ci, dummy_orbs, projector=None):
        ...
        if norb[0] != norb[1]:
            raise RuntimeError(
                "Including padded orbitals doesn't match the number of orbitals in each spin channel!"
                " %d != %d (%d + %d != %d + %d)"
                % (norb[0], norb[1], self.ndummy[0], self.norb[0], self.ndummy[1], self.norb[1])
            )
    ...
    def as_cisdtq(self, *args, **kwargs):
        ...
        c4 = (
            vsl(c4aaaa, [va, va, va, va]),
            vsl(c4aaab, [va, va, va, vb]),
            vsl(c4abab, [va, vb, va, vb]),
            vsl(c4abbb, [va, vb, vb, vb]),
            vsl(c4bbbb, [vb, vb, vb, vb]),
        )
