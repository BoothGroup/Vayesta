...
class Fragmentation_UHF(Fragmentation):
    ...
    def get_frag_coeff(self, indices):
        ...
        c_frag = (self.coeff[0][:, indices].copy(), self.coeff[1][:, indices].copy())
        return c_frag

    def get_env_coeff(self, indices):
        """Get environment coefficients for a given set of orbital indices."""
        env = [np.ones((self.coeff[0].shape[-1]), dtype=bool), np.ones((self.coeff[1].shape[-1]), dtype=bool)]
        env[0][indices] = False
        env[1][indices] = False
        c_env = (self.coeff[0][:, env[0]].copy(), self.coeff[1][:, env[1]].copy())
        return c_env
