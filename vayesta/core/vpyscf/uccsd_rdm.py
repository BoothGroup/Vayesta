...
def _gamma1_intermediates(cc, t1, t2, l1, l2):
    ...
    tmpA += einsum("imef,jmef->ij", l2aa, t2aa) * 0.5

    tmpB = einsum("mief,mjef->ij", l2ab, t2ab)
    tmpB += einsum("imef,jmef->ij", l2bb, t2bb) * 0.5

    tmpC = einsum("mnae,mnbe->ab", t2ab, l2ab)
    tmpC += einsum("mnae,mnbe->ab", t2aa, l2aa) * 0.5

    tmpD = einsum("mnea,mneb->ab", t2ab, l2ab)
    tmpD += einsum("mnae,mnbe->ab", t2bb, l2bb) * 0.5
...
def _gamma2_outcore(cc, t1, t2, l1, l2, h5fobj, compress_vvvv=False):
    ...
    goOvV += einsum("ia,jb->ijab", tmpa, t1b) * 0.5
    goOvV += einsum("ia,jb->jiba", tmpb, t1a) * 0.5
    gOOVV += einsum("ia,jb->ijab", tmpb, t1b)

    tmpa = einsum("kc,kb->cb", l1a, t1a)
    tmpb = einsum("kc,kb->cb", l1b, t1b)
    goovv += einsum("cb,ijca->ijab", tmpa, t2aa) * 0.5
    goOvV -= einsum("cb,ijac->ijab", tmpb, t2ab) * 0.5
    goOvV -= einsum("cb,jica->jiba", tmpa, t2ab) * 0.5
    gOOVV += einsum("cb,ijca->ijab", tmpb, t2bb) * 0.5
    ...
    tmpb += numpy.einsum("ldjd->lj", mIaJb)
    goovv -= einsum("lj,liba->ijab", tmpa, tauaa) * 0.25
    goOvV -= einsum("lj,ilab->ijab", tmpb, tauab) * 0.25
    goOvV -= einsum("lj,liba->jiba", tmpa, tauab) * 0.25
    gOOVV -= einsum("lj,liba->ijab", tmpb, taubb) * 0.25
    tmpa = numpy.einsum("ldlb->db", miajb)
    tmpa += numpy.einsum("ldlb->db", mIaJb)
    tmpb = numpy.einsum("ldlb->db", mIAJB)
    tmpb += numpy.einsum("ldlb->db", miAjB)
    goovv -= einsum("db,jida->ijab", tmpa, tauaa) * 0.25
    goOvV -= einsum("db,ijad->ijab", tmpb, tauab) * 0.25
    goOvV -= einsum("db,jida->jiba", tmpa, tauab) * 0.25
    gOOVV -= einsum("db,jida->ijab", tmpb, taubb) * 0.25

    goovv -= einsum("ldia,ljbd->ijab", miajb, tauaa) * 0.5
    goovv += einsum("LDia,jLbD->ijab", mIAjb, t2ab) * 0.5
    gOOVV -= einsum("ldia,ljbd->ijab", mIAJB, taubb) * 0.5
    gOOVV += einsum("ldia,ljdb->ijab", miaJB, t2ab) * 0.5
    goOvV -= einsum("LDia,LJBD->iJaB", mIAjb, taubb) * 0.25
    goOvV += einsum("ldia,lJdB->iJaB", miajb, t2ab) * 0.25
    goOvV -= einsum("ldIA,ljbd->jIbA", miaJB, tauaa) * 0.25
    goOvV += einsum("LDIA,jLbD->jIbA", mIAJB, t2ab) * 0.25
    goOvV += einsum("lDiA,lJbD->iJbA", miAjB, tauab) * 0.5
    goOvV += einsum("LdIa,jd,LB->jIaB", mIaJb, t1a, t1b) * 0.5

    tmpaa = einsum("klcd,ijcd->ijkl", l2aa, tauaa) * 0.25**2
    tmpbb = einsum("klcd,ijcd->ijkl", l2bb, taubb) * 0.25**2
    tmpabab = einsum("kLcD,iJcD->iJkL", l2ab, tauab) * 0.5
    ...
    goooo = einsum("ijab,klab->ijkl", l2aa, tauaa) * 0.125
    goOoO = einsum("ijab,klab->ijkl", l2ab, tauab) * 0.25
    gOOOO = einsum("ijab,klab->ijkl", l2bb, taubb) * 0.125
    ...
    tmpa = numpy.einsum("icjc->ij", miajb) * 0.25
    tmpa += numpy.einsum("icjc->ij", miAjB) * 0.25
    tmpb = numpy.einsum("icjc->ij", mIAJB) * 0.25
    tmpb += numpy.einsum("icjc->ij", mIaJb) * 0.25
    ...
    gooov += einsum("icja,kc->jkia", miajb, 0.5 * t1a)
    goOoV += einsum("icja,kc->jkia", miAjB, 0.5 * t1b)
    goOoV -= einsum("icJA,kc->kJiA", miaJB, 0.5 * t1a)
    gOoOv += einsum("icja,kc->jkia", mIaJb, 0.5 * t1a)
    gOoOv -= einsum("ICja,KC->KjIa", mIAjb, 0.5 * t1b)
    gOOOV += einsum("icja,kc->jkia", mIAJB, 0.5 * t1b)
    ...
    gOoOv = gOoOv.conj()
    gOOOV = gOOOV.conj()
    gooov += einsum("jkab,ib->jkia", l2aa, 0.25 * t1a)
    goOoV -= einsum("jkba,ib->jkia", l2ab, 0.5 * t1a)
    gOoOv -= einsum("kjab,ib->jkia", l2ab, 0.5 * t1b)
    gOOOV += einsum("jkab,ib->jkia", l2bb, 0.25 * t1b)

    govvv = einsum("ja,ijcb->iacb", 0.25 * l1a, tauaa)
    goVvV = einsum("ja,ijcb->iacb", 0.5 * l1b, tauab)
    gOvVv = einsum("ja,jibc->iacb", 0.5 * l1a, tauab)
    gOVVV = einsum("ja,ijcb->iacb", 0.25 * l1b, taubb)
    ...
    tmpa = numpy.einsum("kakb->ab", miajb) * 0.25
    tmpa += numpy.einsum("kakb->ab", mIaJb) * 0.25
    tmpb = numpy.einsum("kakb->ab", mIAJB) * 0.25
    tmpb += numpy.einsum("kakb->ab", miAjB) * 0.25
    ...
    gOvVv += einsum("kaIB,kc->IaBc", miaJB, 0.5 * t1a)
    gOvVv -= einsum("KaIb,KC->IaCb", mIaJb, 0.5 * t1b)
    gOVVV += einsum("kaib,kc->iabc", mIAJB, 0.5 * t1b)
    govvv = govvv.conj()
    goVvV = goVvV.conj()
    gOvVv = gOvVv.conj()
    gOVVV = gOVVV.conj()
    govvv += einsum("ijbc,ja->iabc", l2aa, 0.25 * t1a)
    goVvV += einsum("iJbC,JA->iAbC", l2ab, 0.5 * t1b)
    gOvVv += einsum("jIcB,ja->IaBc", l2ab, 0.5 * t1a)
    gOVVV += einsum("ijbc,ja->iabc", l2bb, 0.25 * t1b)
    ...
    dovov = (dovov + dovov.transpose(2, 3, 0, 1)) * 0.5
    dvvvv = dvvvv + dvvvv.transpose(1, 0, 3, 2).conj()
    doooo = doooo + doooo.transpose(1, 0, 3, 2).conj()
    dovvo = (dovvo + dovvo.transpose(3, 2, 1, 0).conj()) * 0.5
...
def make_rdm2(mycc, t1, t2, l1, l2, ao_repr=False, with_frozen=True, with_dm1=True):
    ...
    return _make_rdm2(mycc, d1, d2, with_dm1=with_dm1, with_frozen=with_frozen, ao_repr=ao_repr)
...
def _make_rdm2(mycc, d1, d2, with_dm1=True, with_frozen=True, ao_repr=False):
    ...
    if with_frozen and mycc.frozen is not None:
        ...
        lib.takebak_2d(rdm2aa.reshape(nmoa**2, nmoa**2), dm2aa.reshape(nmoa0**2, nmoa0**2), idxa, idxa)
        lib.takebak_2d(rdm2bb.reshape(nmob**2, nmob**2), dm2bb.reshape(nmob0**2, nmob0**2), idxb, idxb)
        lib.takebak_2d(rdm2ab.reshape(nmoa**2, nmob**2), dm2ab.reshape(nmoa0**2, nmob0**2), idxa, idxb)
...
def _dm2ab_mo2ao(dm2, mo_a, mo_b):
    return lib.einsum("ijkl,pi,qj,rk,sl->pqrs", dm2, mo_a, mo_a.conj(), mo_b, mo_b.conj())
...
if __name__ == "__main__":
    ...
    e1 += numpy.einsum("ijkl,ijkl", eriaa, dm2aa) * 0.5
    e1 += numpy.einsum("ijkl,ijkl", eriab, dm2ab)
    e1 += numpy.einsum("ijkl,ijkl", eribb, dm2bb) * 0.5
    e1 += mol.energy_nuc()
    print(e1 - mycc.e_tot)
    ...
    mol.basis = "6-31g"
    mol.build()
    mf = scf.UHF(mol).run(init_guess="hcore", conv_tol=1.0)
