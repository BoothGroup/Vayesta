...
def _gamma2_intermediates(mycc, t1, t2, l1, l2, compress_vvvv=False):
    ...
    d2 = (f["dovov"][:], f["dvvvv"][:], f["doooo"][:], f["doovv"][:], f["dovvo"][:], None, f["dovvv"][:], f["dooov"][:])
    return d2
...
def _gamma2_outcore(mycc, t1, t2, l1, l2, h5fobj, compress_vvvv=False):
    ...
    dovvo = h5fobj.create_dataset("dovvo", (nocc, nvir, nvir, nocc), dtype, chunks=(nocc, 1, nvir, nocc))
    fswap = lib.H5TmpFile()
    ...
    mia = numpy.einsum("kc,ikac->ia", l1, t2) * 2 - numpy.einsum("kc,ikca->ia", l1, t2)
    mab = numpy.einsum("kc,kb->cb", l1, t1)
    mij = numpy.einsum("kc,jc->jk", l1, t1) + moo * 0.5

    tau = numpy.einsum("ia,jb->ijab", t1, t1)
    tau += t2
    goooo = lib.einsum("ijab,klab->ijkl", tau, l2) * 0.5
    h5fobj["doooo"] = (goooo.transpose(0, 2, 1, 3) * 2 - goooo.transpose(0, 3, 1, 2)).conj()

    gooov += numpy.einsum("ji,ka->jkia", -0.5 * moo, t1)
    ...
    unit = nocc**2 * nvir * 6
    blksize = min(nocc, nvir, max(ccsd.BLKMIN, int(max_memory * 0.95e6 / 8 / unit)))
    doovv = h5fobj.create_dataset("doovv", (nocc, nocc, nvir, nvir), dtype, chunks=(nocc, nocc, 1, nvir))
    ...
    for p0, p1 in lib.prange(0, nvir, blksize):
        ...
        tmpoovv -= lib.einsum("bd,ijad->ijab", mvv * 0.5, tau)
        tmpoovv += 0.5 * tau
        ...
        for q0, q1 in lib.prange(0, nvir, blksize):
            ...
            tmp = pvoOV[:, :, :, q0:q1] + pvOOv[:, :, :, q0:q1] * 0.5
    ...
    max_memory = max(0, mycc.max_memory - lib.current_memory()[0])
    unit = max(nocc**2 * nvir * 2 + nocc * nvir**2 * 3, nvir**3 * 2 + nocc * nvir**2 * 2 + nocc**2 * nvir * 2)
    blksize = min(nvir, max(ccsd.BLKMIN, int(max_memory * 0.9e6 / 8 / unit)))
    log.debug1(
    ...
    dovvv = h5fobj.create_dataset("dovvv", (nocc, nvir, nvir, nvir), dtype, chunks=(nocc, min(nocc, nvir), 1, nvir))
    time1 = logger.process_clock(), logger.perf_counter()
    for istep, (p0, p1) in enumerate(lib.prange(0, nvir, blksize)):
        ...
        if compress_vvvv:
            ...
            for i in range(p1 - p0):
                ...
                tmp[:] = vvv - vvv.transpose(2, 1, 0) * 0.5
            ...
            for i in range(p0, p1):
                ...
                dvvvv[off : off + i + 1] = tmpvvvv[i - p0, : i + 1] * 0.25
            tmp = tmpvvvv = None
        else:
            for i in range(p0, p1):
        ...
        gvovv += numpy.einsum("ba,ic->aibc", mvv[:, p0:p1] * 0.5, t1)
        gvovv = gvovv.conj()
...
def _make_rdm2(mycc, d1, d2, with_dm1=True, with_frozen=True, ao_repr=False):
    ...
    if with_frozen and mycc.frozen is not None:
        ...
        lib.takebak_2d(rdm2.reshape(nmo**2, nmo**2), dm2.reshape(nmo0**2, nmo0**2), idx, idx)
        dm2 = rdm2
...
if __name__ == "__main__":
    ...
    h1 = fock0 - (numpy.einsum("kkpq->pq", eri0[:nocc, :nocc]) * 2 - numpy.einsum("pkkq->pq", eri0[:, :nocc, :nocc]))
    ...
    fdm2 = lib.H5TmpFile()
    dovov, dvvvv, doooo, doovv, dovvo, dvvov, dovvv, dooov = _gamma2_outcore(mcc, t1, t2, l1, l2, fdm2, True)
    ...
    dovov, dvvvv, doooo, doovv, dovvo, dvvov, dovvv, dooov = _gamma2_intermediates(mcc, t1, t2, l1, l2)
    print("dovov", lib.finger(numpy.array(dovov)) - -14384.907042073517)
    ...
    print(numpy.einsum("pqrs,pqrs", dm2, eri0) * 0.5 + numpy.einsum("pq,qp", dm1, h1) - e2)
    ...
    e1 += numpy.einsum("ijkl,ijkl", eri, dm2) * 0.5
    e1 += mol.energy_nuc()
    print(e1 - mycc.e_tot)
