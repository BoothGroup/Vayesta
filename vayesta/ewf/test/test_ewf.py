...
def test_fci_solver(a=2.0, kmesh=[2,2,2], bno_threshold=1e-4):
    ...
    t0 = timer()
    ecc = pyscf.embwf.EmbWF(kmf, solver='FCI', bno_threshold=bno_threshold)
...
def test_cubic(atom='He', a=2.0, basis='gth-dzv', kmesh=[2,2,2], bno_threshold=1e-5):
    ...
    t0 = timer()
    ecc = pyscf.embwf.EmbWF(kmf, bno_threshold=bno_threshold)
    ecc.make_atom_cluster(0)
    ecc.kernel()
    t_ecc = timer()-t0
    print("E(Emb-CCSD)= %+16.8f Ha" % ecc.e_tot)
...
def test_canonical_orth(c=1.2, lindep_threshold=1e-8, kmesh=[1,1,2], output=None):
    ...
    #kcc = pyscf.embwf.EmbWF(kmf, bno_threshold=-1)
...
#    kmf.kernel()
#
#    kcc = pyscf.embwf.EmbWF(kmf, bath_tol=bath_tol, bno_threshold=bno_threshold)
...
#    kcc.kernel()
#    print("Time for k-EmbWF= %.3f" % (timer()-t0))
#    print("k-EmbWF E= %16.8g" % kcc.e_tot)
#    if EXPECTED:
...
#    smf.kernel()
#
#    scc = pyscf.embwf.EmbWF(smf, bath_tol=bath_tol, bno_threshold=bno_threshold)
#    scc.opts.popfile = None
#    scc.opts.orbfile = None
#    scc.make_atom_cluster(0, symmetry_factor=2*ncells)
#    t0 = timer()
#    scc.kernel()
#    print("Time for supercell-EmbWF= %.3f" % (timer()-t0))
...
#    t0 = timer()
#    kcc.kernel()
#    print("Time for k-EmbWF (incore GDF)= %.3f" % (timer()-t0))
#    print("k-EmbWF (incore GDF) E= %16.8g" % kcc.e_tot)
#    if EXPECTED:
#        assert np.isclose(kcc.e_tot, EXPECTED)
...
def test_full_ccsd_limit(EXPECTED, kmesh=[2, 2, 2]):
    ...
    #kcc = pyscf.embwf.EmbWF(kmf, bath_tol=-1)
    kcc = pyscf.embwf.EmbWF(kmf, bno_threshold=[-1])
    kcc.opts.popfile = None
    kcc.opts.orbfile = None
    kcc.make_atom_cluster(0, sym_factor=2)
