...
def make_rdm1_ccsd(
    emb, ao_basis=False, t_as_lambda=None, symmetrize=True, with_mf=True, mpi_target=None, mp2=False, ba_order="ba"
):
    ...
    if mpi:
        if mp2:
            dooa, doob, dvva, dvvb = mpi.nreduce(dooa, doob, dvva, dvvb, target=mpi_target, logfunc=emb.log.timingv)
        else:
...
def make_rdm1_ccsd_global_wf(
    ...
    for x in emb.get_fragments(contributes=True, mpi_rank=mpi.rank, **xfilter):
        ...
        for y in emb.get_fragments(contributes=True):
            ...
            if svd_tol is not None:
                ...
                nsv = min(rxy_occ_a.shape[0], rxy_occ_a.shape[1]) + min(rxy_vir_a.shape[0], rxy_vir_a.shape[1])
                total_sv = kept_sv = total_sv + nsv
                nsv = min(rxy_occ_b.shape[0], rxy_occ_b.shape[1]) + min(rxy_vir_b.shape[0], rxy_vir_b.shape[1])
                total_sv = kept_sv = total_sv + nsv
            ...
            if not late_t2_sym:
                if svd_tol is None:
                    ...
                    tlaa = einsum(
                        "(ijab,jX,bY->iXaY),(XJ,YB,IJAB)->iIaA", t2aa, uxy_occ_a, uxy_vir_a, vxy_occ_a, vxy_vir_a, l2aa
                    )
                    tlbb = einsum(
                        "(ijab,jX,bY->iXaY),(XJ,YB,IJAB)->iIaA", t2bb, uxy_occ_b, uxy_vir_b, vxy_occ_b, vxy_vir_b, l2bb
                    )
                    tlab1 = einsum(
                        "(ijab,jX,bY->iXaY),(XJ,YB,IJAB)->iIaA", t2ab, uxy_occ_b, uxy_vir_b, vxy_occ_b, vxy_vir_b, l2ab
                    )
                    tlab2 = einsum(
                        "(ijab,iX,aY->XjYb),(XI,YA,IJAB)->jJbB", t2ab, uxy_occ_a, uxy_vir_a, vxy_occ_a, vxy_vir_a, l2ab
                    )
        ...
        if use_sym:
            ...
            for x2, (
                ...
                dvvxa2,
                dvvxb2,
            ) in x.loop_symmetry_children(arrays, axes=[0, 0, 0, 0, 1, 1, 1, 1]):
        ...
        if with_t1:
            ...
            if use_sym:
                ...
                for x2, (
                    ...
                    cx2_occ_b,
                    cx2_vir_b,
                ) in x.loop_symmetry_children(arrays, include_self=False, maxgen=None):
    ...
    if mpi:
        rma.clear()
        dooa, doob, dvoa, dvob, dvva, dvvb = mpi.nreduce(
            dooa, doob, dvoa, dvob, dvva, dvvb, target=mpi_target, logfunc=emb.log.timingv
        )
...
def make_rdm2_ccsd_global_wf(emb, ao_basis=False, symmetrize=False, t_as_lambda=None, slow=True, with_dm1=True):
...
def make_rdm2_ccsd_proj_lambda(emb, ao_basis=False, t_as_lambda=None, with_dm1=True, mpi_target=None):
