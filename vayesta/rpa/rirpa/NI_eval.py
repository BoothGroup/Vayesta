...
class NumericalIntegratorBase:
    ...
    def test_diag_derivs(self, a, delta=1e-6):
        ...
        self.log.info(
            "Testing gradients w.r.t variation of omega at random frequency point=%8.6e:",
            freq,
        )
    ...
    def opt_quadrature_diag(self, ainit=None):
        ...
        try:
            solve, res = scipy.optimize.newton(
                get_val,
                x0=ainit,
                fprime=get_grad,
                tol=1e-8,
    ...
    def kernel_adaptive(self):
        self.fix_params()
        integral, err, info = scipy.integrate.quad_vec(
            self.eval_contrib,
            a=0.0,
            b=np.inf,
            norm="max",
...
class NumericalIntegratorClenCur(NumericalIntegratorBase):
    ...
    def npoints(self, value):
        if value % 4 != 0:
            value += 4 - value % 4
            self.log.warning(
                "Npoints increased to next multiple of 4 (%d) to allow error estimation.",
                value,
            )
        self._npoints = value
...
class NumericalIntegratorGaussianSemiInfinite(NumericalIntegratorBase):
    ...
    def npoints(self, value):
        ...
        if value > 100:
            self.log.warning(
                "Gauss-Laguerre quadrature with degree over 100 may be problematic due to numerical "
