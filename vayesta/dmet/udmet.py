...
class UDMET(UEmbedding, RDMET):
    ...
    def update_vcorr(self, fock, curr_rdms):
        ...
        vcorr_new = np.array(
            (
                perform_SDP_fit(
                    self.mol.nelec[0], fock[0], impurity_coeffs[0], [x[0] for x in curr_rdms], self.get_ovlp(), self.log
                ),
                perform_SDP_fit(
                    self.mol.nelec[1], fock[1], impurity_coeffs[1], [x[1] for x in curr_rdms], self.get_ovlp(), self.log
                ),
            )
        )
    ...
    def get_impurity_coeffs(self):
        ...
        return [
            [
                [parent.c_frag[x]] + [c.c_frag[x] for c in children]
                for (parent, children) in zip(sym_parents, sym_children)
            ]
            for x in [0, 1]
        ]
